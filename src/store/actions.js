...
//   await axios.put(`${BASE_API_URL}/products/update/${id}/`, product);
  ...
  return async (dispatch) => {
    try {
      const response = await axios.delete(
        `${BASE_API_URL}/products/delete/${id}`
      );
...
export const editProducts = (payload, id) => {
  return async (dispatch) => {
    ...
    Object.keys(payload).forEach((key) => {
      if (key === "image") {
        formData.append(key, payload[key]?.file);
        // formData.append(key, payload[key]?.image / png);
      } else {
        formData.append(key, payload[key]);
      }
    });
    ...
    try {
      const response = await axios.put(
        `${BASE_API_URL}/products/update/${id}/`,
        data
      );
